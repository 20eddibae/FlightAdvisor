...
export default function RouteControls({
  ...
  /* State to track if the user has manually successfully planned a route at least once.
   * Auto-recalculation should only be active AFTER the initial manual trigger.
   */
  ...
  const handlePlanRoute = () => {
    ...
    // Update ref to current parameters to prevent immediate double-trigger
    lastPlannedRef.current = JSON.stringify({ dep: departure.id, dest: destination.id, len: maxLen })

    onPlanRoute(departure.id, destination.id, maxLen)
  }

  useEffect(() => {
    // Only auto-calculate if we've already done an initial search
    if (hasInitialSearch && departure && destination) {
      const maxLen = maxSegmentLength ? parseInt(maxSegmentLength, 10) : undefined
      const currentParams = JSON.stringify({ dep: departure.id, dest: destination.id, len: maxLen })

      // Prevent loop: If these parameters match the last successfully planned route, do nothing.
      if (currentParams === lastPlannedRef.current) {
        return
      }

      const timer = setTimeout(() => {
        lastPlannedRef.current = currentParams
        onPlanRoute(departure.id, destination.id, maxLen)
      }, 500)

      return () => clearTimeout(timer)
    }
  }, [departure, destination, maxSegmentLength, onPlanRoute, hasInitialSearch])
  ...
  return (
    <>
      {isCalculating && (
        <div className="fixed top-0 left-0 right-0 h-1 md:h-1.5 z-[100] bg-blue-100/20 pointer-events-none overflow-hidden">
          <div className="h-full bg-blue-600 shadow-[0_0_15px_rgba(37,99,235,0.6)] animate-view-loading"></div>
          <style jsx global>{`
            @keyframes view-loading {
              0% { transform: translateX(-100%); width: 40%; }
              50% { width: 60%; }
              100% { transform: translateX(300%); width: 40%; }
            }
            .animate-view-loading {
              animation: view-loading 1.2s infinite linear;
            }
          `}</style>
        </div>
      )}
